...
# COIN = Greenblocks
# DB_DIRECTORY = /opt/greenblocks/electrumx/db/
# DAEMON_URL = http://123:123123123@localhost:42003/
# TCP_PORT = 8884
# RPC_PORT = 9009
...
class Env(EnvBase):
    ...
    def __init__(self, coin=None):
        ...
        if coin is not None:
            ...
            coin_name = self.default('COIN','Scribe')
            network = self.default('NET', 'mainnet')
            self.coin = Coin.lookup_coin_class('Scribe', 'mainnet')
        self.cache_MB = self.integer('CACHE_MB', 1200)
        self.host = self.default('HOST', '0.0.0.0')
        self.reorg_limit = self.integer('REORG_LIMIT', self.coin.REORG_LIMIT)
        # Server stuff
        self.tcp_port = self.integer('TCP_PORT', 33001)
        self.ssl_port = self.integer('SSL_PORT', None)
        if self.ssl_port:
            self.ssl_certfile = self.required('SSL_CERTFILE')
            self.ssl_keyfile = self.required('SSL_KEYFILE')
        self.rpc_port = self.integer('RPC_PORT', 34001)
        ...
        self.max_subs = self.integer('MAX_SUBS', 2500000)
